...
| Auth/authz | `test_auth.py` | Every protected endpoint: missing key → `401`, invalid key → `401`, wrong role → `403`, correct role passes the gate; `GET /health`/`GET /audit/export/public-key` need no key; `FIELD_REDACTED`/`RECORD_ARCHIVED` system events carry the authenticated principal's id, not caller input (C10); a spoofed `actorId` on redact is rejected (`422`), not silently dropped; `GET /audit/verify` stays unscoped even for a scoped principal (C12) |
| Cross-account denial | `test_cross_tenant.py` | A `resourceScope`-restricted principal (C12) reaches only its own account via query/export, is denied (`404`) naming another account explicitly, when omitting the filter entirely, and when filtering by a `resourceType` shared with the other account — proving the intersection is server-side and keyed on `resourceId` specifically; denial holds symmetrically for a second scoped principal |
